...
class UrllibClient(BaseTester):
    ...
    def _get_request(self, data, endpoint):
        ...
        response = self.pool.request("GET", final_url, headers={"X-MBX-APIKEY": self.apiKey})
        return response.json()
    
    def _post_request(self, data, endpoint):
        final_url = self._get_final_url(data, endpoint)
        response = self.pool.request("POST", final_url, headers={"X-MBX-APIKEY": self.apiKey})
        return response.json()
    
    def _delete_request(self, data, endpoint):
        final_url = self._get_final_url(data, endpoint)
        response = self.pool.request("DELETE", final_url, headers={"X-MBX-APIKEY": self.apiKey})
        return response.json()
